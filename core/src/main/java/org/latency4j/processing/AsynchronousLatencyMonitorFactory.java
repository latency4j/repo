...
public class AsynchronousLatencyMonitorFactory implements MonitorFactory {
	...
	/**
	 * <p>
	 * A reference to the {@link Latency4JResourceManager Latency4J resource
		...
		if (result == null) {
			...
			try {
				...
				if (!this.monitorMap.containsKey(workCategory)) {
					LatencyRequirement requirement = getLatency4JResourceManager().getLatencyRequirement(workCategory);
	 ...
	 *            The path to the {@link Latency4JConfiguration Latency4J
	 *            configuration}.
	 */
	 ...
	 * the {@link Latency4JConfiguration Latency4J configuration} from the first
	...
	public void init() {
		...
		try {
			ConfigurationReader.readConfiguration(getLatency4JResourceManager(), configurationResourceName);
			...
			if (useDefault) {
				...
				logger.warn("Latency4J not initialized! No configuration set found at this time");
	 ...
	 *            {@link Latency4JConfiguration Latency4J configuration} and
	 *            initialise this instance.
	 * 
	 * @see MonitorFactoryStaticHandle#initializeFrom(InputStream)
	 */
	public void init(final InputStream stream) {
		ConfigurationReader.readConfiguration(getLatency4JResourceManager(), stream);
	}
	 ...
	 *         manager} for this instance.
	 */
	protected Latency4JResourceManager getLatency4JResourceManager() {
